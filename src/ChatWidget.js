...
export default function ChatWidget({ onClose }) {
  ...
  const openWhatsApp = () => {
    ...
    const phoneNumber = '79001234567'; // Замените на реальный номер
    ...
    // TODO: Получить данные заказа из localStorage или props
    const orderData = localStorage.getItem('orderData') ? JSON.parse(localStorage.getItem('orderData')) : null;
    
    // Формируем сообщение с информацией о чате
    let chatInfo = `Здравствуйте! 👋

У меня есть вопросы по заказу.
...
• Сообщений в чате: ${messages.length}`;
    ...
    if (orderData) {
      chatInfo += `
📦 Данные заказа:
• Продукт: ${orderData.product || 'Не указан'}
• Стоимость: ${orderData.price || 'Не указана'}
• Дата: ${orderData.date || 'Не указана'}`;
    }
...
Хотел бы продолжить обсуждение в WhatsApp.`;

    // Создаем ссылку WhatsApp
    const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(chatInfo)}`;
    ...
    <div style={fullscreen || isMobile ? styles.overlayFull : styles.overlay}>
      <div style={fullscreen || isMobile ? styles.chatBoxFull : styles.chatBox}>
        <div style={styles.header}>
          ...
          <button onClick={openWhatsApp} style={styles.whatsappBtn} title="Перейти в WhatsApp">
...
const styles = {
  ...
  whatsappBtn: {
    background: 'none', border: 'none', cursor: 'pointer', marginRight: 8, opacity: 0.8, transition: 'opacity 0.2s',
